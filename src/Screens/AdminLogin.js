...
function AdminLogin() {
    ...
    const formik = useFormik({
      ...
      onSubmit: async (values) => {
        try {
         let loginData =  await axios.post("http://localhost:3001/users/adminlogin", values);
          
         window.localStorage.setItem("myapptoken", loginData.data.token)
          navigate("/users/adminpannel");
        } catch (error) {
          console.log(error)
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="row">
          ...
          <div className="col-lg-12">
            ...
            <p>New Admin?<Link to={'/users/adminregister'}>Register here</Link></p>
          </div>
        </div>
      </form>
    </div>
